    ...
    <table class="tablelist">
        ...
        <?php if(is_array($arr)): foreach($arr as $key=>$vo): ?><tr class="tr" id="0_1" user_id="<?php echo ($vo["user_id"]); ?>">
                ...
                <td><a href="<?php echo U('User/user_update');?>?user_id=<?php echo ($vo["user_id"]); ?>" class="tablelink">修改</a>
                    ...
                    <a href="<?php echo U('User/admin_user_campus');?>?user_id=<?php echo ($vo["user_id"]); ?>" class="tablelink"> 拥有店铺</a>
                    <a href="<?php echo U('Pay/pay_add');?>?user_id=<?php echo ($vo["user_id"]); ?>" class="tablelink"> 充值</a>
                    <?php if($vo['open_close'] == 1): ?><font><a href="#" location="<?php echo ($vo["open_close"]); ?>" id="<?php echo ($vo["user_id"]); ?>" class="tablelink location" style="color: red;">校区关闭</a></font>
                        <?php else: ?>
                        <font><a href="#" location="<?php echo ($vo["open_close"]); ?>" id="<?php echo ($vo["user_id"]); ?>" class="tablelink location" style="color: orange;">校区开启</a></font><?php endif; ?>
                </td>
    ...
    $(function(){
        $(document).on("click",".location",function(){
            ...
            $.ajax({
                ...
                success:function(r){
                    if(r==1){
                        object.parent().html('<a href="#" location="2" id="'+id+'" class="tablelink location" style="color: orange;">校区开启</a>')
                    }else if(r==2){
                        object.parent().html('<a href="#" location="1" id="'+id+'" class="tablelink location" style="color: red  ;">校区关闭</a>')
                    }
                }
            })
        })
